...
ADD COLUMN IF NOT EXISTS parent_comment_id UUID REFERENCES video_comments(id) ON DELETE CASCADE;
...
CREATE POLICY IF NOT EXISTS "Les utilisateurs peuvent voir tous les likes de commentaires"
  ON comment_likes FOR SELECT
  USING (true);

-- Politique RLS: Les utilisateurs peuvent liker
CREATE POLICY IF NOT EXISTS "Les utilisateurs peuvent liker des commentaires"
  ON comment_likes FOR INSERT
  WITH CHECK (auth.uid() = user_id);

-- Politique RLS: Les utilisateurs peuvent unliker
CREATE POLICY IF NOT EXISTS "Les utilisateurs peuvent supprimer leurs likes de commentaires"
...
-- Politique RLS: Tout le monde peut voir les likes
CREATE POLICY IF NOT EXISTS "Les utilisateurs peuvent voir tous les likes de commentaires de vidéos"
  ON video_comment_likes FOR SELECT
  USING (true);

-- Politique RLS: Les utilisateurs peuvent liker
CREATE POLICY IF NOT EXISTS "Les utilisateurs peuvent liker des commentaires de vidéos"
  ON video_comment_likes FOR INSERT
  WITH CHECK (auth.uid() = user_id);

-- Politique RLS: Les utilisateurs peuvent unliker
CREATE POLICY IF NOT EXISTS "Les utilisateurs peuvent supprimer leurs likes de commentaires de vidéos"
