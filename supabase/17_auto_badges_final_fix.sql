-- =============================================
-- CORRECTION FINALE - Clause ON CONFLICT
-- Description: Corrige l'ambiguïté dans la clause ON CONFLICT
-- =============================================

-- Corriger la fonction check_and_assign_badges avec la clause ON CONFLICT corrigée
...
BEGIN
  ...
  LOOP
    ...
    IF v_should_have_badge AND NOT v_has_badge THEN
      -- Attribuer le badge (vérifier d'abord pour éviter l'ambiguïté dans ON CONFLICT)
      BEGIN
...
AND routine_name = 'check_and_assign_badges';
